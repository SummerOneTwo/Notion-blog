...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  // 防御性检查：如果 id 不是有效的 Notion page ID 格式，直接返回 null
  // 有效的 Notion ID 应该是 32 位十六进制字符串或带连字符的 UUID 格式
  if (!id || typeof id !== 'string' || id.startsWith('error-page-') || !/^[a-f0-9-]{32,36}$/i.test(id.replace(/-/g, ''))) {
    console.warn('[API跳过]', `无效的页面ID: ${id}`)
    return null
  }
