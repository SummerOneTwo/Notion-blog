...
export function generateLocaleDict(langString) {
  ...
  // 防御性检查：确保 langString 是字符串
  const [language, region] = (typeof langString === 'string' && langString) ? langString.split(/[-_]/) : ['en', 'US']
...
export const redirectUserLang = (lang, pageId) => {
  ...
  // 没有开启多语言
  const notionPageId = BLOG.NOTION_PAGE_ID
  if (!notionPageId || typeof notionPageId !== 'string' || notionPageId.indexOf(',') < 0) {
    return
  }
