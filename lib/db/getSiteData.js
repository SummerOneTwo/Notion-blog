...
export async function getGlobalData({
  ...
  // 防御性检查：确保 pageId 是字符串
  const siteIds = (typeof pageId === 'string' && pageId) ? pageId.split(',') : []
...
const EmptyData = pageId => {
  // 当 Notion API 获取失败时，返回空数据
  // 不再返回假页面，避免触发渲染错误
  console.warn(`[EmptyData] 无法获取Notion数据，pageId: ${pageId}`)
  const empty = {
    notice: null,
    siteInfo: getSiteInfo({}),
    allPages: [], // 返回空数组，不再包含假页面
    allNavPages: [],
    collection: [],
    collectionQuery: {},
    collectionId: null,
...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  ...
  // 防御性检查：确保 collection 存在
  const collectionObj = pageRecordMap.collection
  const collection = (collectionObj && typeof collectionObj === 'object')
    ? (Object.values(collectionObj)[0]?.value || {})
    : {}
